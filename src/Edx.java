...
public class Edx
{
	public static void main(String args[]) throws IOException
	{
     ...
     FileWriter writer_prof = new FileWriter(sFileProf);
    	...
    	Elements Course_images = doc.select("div.image a img");
    		...
    		{
    			String innerUrl = (String) Course_url.get(i).attr("abs:href");
                ...
                String titles = Titles.get(i).text();
                String short_desc = shortDescription.get(i).text();
                String long_desc = innerDoc.select("div.course-detail-about").text();
                String course_url = Course_url.get(i).attr("abs:href");
                String videolink = innerDoc.select("div.course-detail-video a[href]").attr("abs:href");
            	if(innerDoc.select("div.course-detail-video a[href]").attr("abs:href").isEmpty())
            	{
            		videolink = "N/A";
            	}
            	String date = innerDoc.select("div.course-detail-start").text();
            	String course_length = innerDoc.select("div.course-detail-length").text();
            	...
            	else
            	{
            		...
            		course_length = course_length.substring(course_length.indexOf(":")+2, course_length.indexOf(":")+3);
            		
            	}
                String course_images = Course_images.attr("src");
                String category = "N/A";
                String site_url = "https://edx.org/";
                String price = "0";
                String language = "English";
                String certificate = "yes";
                String university = innerDoc.select("div.course-detail-school").text();
                ...
                {
                    String staff_name = staff.get(j).text().replace(",", "");
            ...
            String StrDate = date.substring(date.indexOf(":")+1, date.length()); 
			StrDate=StrDate.trim();
			String OriginalDate = StrDate;
